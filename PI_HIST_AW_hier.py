...
if __name__ == '__main__':
    ...
    num_AWs = len(AW_list)
    print('#NODES %d #EDGS %d L=%d #AWs %d'
          % (num_nodes, num_edges, RW_len, num_AWs))
    ...
    # ====================
    ttl_comp_time = 0.0 # Total computation time
    ttl_IO_comm_time = 0.0 # Total I/O & communication time
    ...
    for iter in range(num_iter):
        ...
        time_e = timeit.default_timer()
        AW_time = time_e - time_s
        ttl_comp_time += AW_time
        del RWs_tnr
        ...
        time_e = timeit.default_timer()
        idx_time = time_e - time_s
        ttl_comp_time += idx_time
        ...
        ttl_comp_time += cnt_time
        # ====================
        print('ITER %d/%d: RW %f AW-MAP %f AW-IDX %f CNT %f'
              % (iter+1, num_iter, RW_time, AW_time, idx_time, cnt_time))
    ...
    ttl_comp_time += gat_time
    print('GAT %f' % (gat_time))

    idxs = AW_stat_sp_gbl.indices()
    vals = AW_stat_sp_gbl.values()
    #print(AW_stat_sp_gbl)
    del AW_stat_sp_gbl
    torch.cuda.empty_cache()
    gc.collect()
    ...
    ttl_comp_time += hier_time
    print('HIER TIME %f' % (hier_time))

    print('TTL TIME %f' % (ttl_comp_time))
    #print('IO & COMM TIME %f' % (ttl_IO_comm_time))

    # ====================
    pkl_file = open('AW_hier/AW_hier_bth_%s_L=%d_n=%d.pickle' % (data_name, RW_len, num_RWs), 'wb')
    pickle.dump(adj_bth_idxs, pkl_file)
    pkl_file.close()
    # ==========
    pkl_file = open('AW_hier/AW_hier_src_%s_L=%d_n=%d.pickle' % (data_name, RW_len, num_RWs), 'wb')
    pickle.dump(adj_src_idxs, pkl_file)
    pkl_file.close()
    # ==========
    pkl_file = open('AW_hier/AW_hier_dst_%s_L=%d_n=%d.pickle' % (data_name, RW_len, num_RWs), 'wb')
    pickle.dump(adj_dst_idxs, pkl_file)
    pkl_file.close()
    # ==========
    pkl_file = open('AW_hier/AW_hier_vals_%s_L=%d_n=%d.pickle' % (data_name, RW_len, num_RWs), 'wb')
    pickle.dump(adj_vals, pkl_file)
    pkl_file.close()
