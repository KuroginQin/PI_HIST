...
if __name__ == '__main__':
    ...
    # ==========
    res_df['AUC_orig'] = res_df['AUC'].astype(str)
    oom_oot_mask = res_df['AUC_orig'].isin(['OOM', 'OOT'])

    # ====================
    # Select valid methods & OOM/OOT methods
    val_res = res_df.loc[~oom_oot_mask, 'AUC_orig'].astype(float)
    min_val_res = val_res.min() if not val_res.empty else 0
    # ==========
    res_df.loc[oom_oot_mask, 'AUC'] = min_val_res - 0.2
    res_df.loc[~oom_oot_mask, 'AUC'] = res_df.loc[~oom_oot_mask, 'AUC_orig'].astype(float)
    # ==========
    ...
    # ==========
    oom_oot_info = []
    for idx, (is_oom_oot, text) in enumerate(zip(oom_oot_mask, res_df['AUC_orig'])):
        if is_oom_oot:
            oom_oot_info.append((idx, text))
    # ====================
    ...
    bars = ax1.barh(meth_y, qlt_res_list, height=0.8, edgecolor='white', linewidth=0.5)
    ...
    # ====================
    # Draw OOT/OOM text in histogram
    for pos, text in oom_oot_info:
        txt = ax1.text(
            x=res_df.loc[pos, 'AUC'],
            y=pos,
            s=text,
            rotation=0,
            ha='center',
        ...
        dx_pixels = 38
        offset = mtransforms.offset_copy(ax1.transData, fig=fig, x=dx_pixels, y=0, units='dots')
    ...
    x_min = min_val_res - 0.2
    x_max = max(qlt_res_list) + 0.1
